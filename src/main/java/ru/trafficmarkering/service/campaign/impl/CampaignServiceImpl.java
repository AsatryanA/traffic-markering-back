...
class CampaignServiceImpl implements CampaignService {
    ...
    private void resetRegionViews(UUID campaignId) {
        // Одним UPDATE, а не save() по каждому отклику: объявление с сотнями откликов
        // не должно держать транзакцию по одной вспышке (saveAndFlush) на каждый ряд
        saverApplication.resetRegionViewsByCampaignId(campaignId);
    }
